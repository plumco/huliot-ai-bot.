                ...
                if response.status_code != 200:
                    ...
                    if response.status_code == 429:
                        st.error(
                            ...
                            "Check your Google Cloud billing and quota for the Gemini project, "
                            "or wait and retry after the rate limit resets."
                        )
                        st.write(message)
                    else:
                        st.error(f"Gemini request failed: {response.status_code} - {message}")
                else:
                    data = response.json()
                    answer = None
